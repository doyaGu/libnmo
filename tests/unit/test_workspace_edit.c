...
TEST(workspace_edit, manager_edits_reject_invalid_identifier_links) {
    for (size_t attribute_manager = 0u; attribute_manager < 2u;
        ...
        ASSERT_EQ(NMO_OK, nmo_chunk_write_identifier(
            chunk, attribute_manager != 0u ? 0x52u : 0x53u));
        ASSERT_EQ(NMO_OK, nmo_chunk_write_int(chunk, 0));
        if (attribute_manager != 0u) {
            ASSERT_EQ(NMO_OK, nmo_chunk_write_int(chunk, 0));
        }
        nmo_chunk_close(chunk);
        ...
        nmo_workspace_edit_t *edit = NULL;
        ASSERT_EQ(NMO_OK, begin_workspace_edit_for_session(
            ctx, session, "invalid manager identifier", &scope, &edit));
        uint32_t value = UINT32_MAX;
        nmo_status_t status = attribute_manager != 0u
...
TEST(workspace_edit, manager_edits_reject_cross_section_strings) {
    for (size_t attribute_manager = 0u; attribute_manager < 2u;
         ++attribute_manager) {
        nmo_context_t *ctx = nmo_context_create(&(nmo_context_desc_t){0});
        ASSERT_NOT_NULL(ctx);
        ...
        nmo_workspace_edit_t *edit = NULL;
        ASSERT_EQ(NMO_OK, begin_workspace_edit_for_session(
            ctx, session, "cross-section manager string", &scope, &edit));
        uint32_t value = UINT32_MAX;
        nmo_status_t status = attribute_manager != 0u
            ? nmo_object_edit_ensure_attribute_manager_entry(
                  edit, "new-attribute", NULL, &value)
            : nmo_object_edit_ensure_message_manager_entry(
                  edit, "new-message", &value);
        ASSERT_EQ(NMO_ERR_TRUNCATED_CHUNK, status);
        ASSERT_EQ(UINT32_MAX, value);
        rollback_workspace_edit_scope(&scope);
...
TEST(workspace_edit, parameter_write_creates_attribute_manager_entry_transactionally) {
    ...
    nmo_object_id_t param_id = 0;
    create_object_or_fail(session, NMO_CID_PARAMETER, "attribute-param", &param_id);
    nmo_object_t *param_obj =
        nmo_object_repository_find_by_id(nmo_session_get_repository(session), param_id);
    ASSERT_NOT_NULL(param_obj);
    nmo_parameter_state_t *state = nmo_parameter_get_mutable_state(param_obj);
    ...
    workspace_edit_scope_t lookup_scope = {0};
    nmo_workspace_edit_t *lookup_edit = NULL;
    nmo_parameter_write_options_t lookup_options = {
        .manager_entry = {
    ...
    ASSERT_EQ(NMO_OK,
              begin_workspace_edit_for_session(
                  ctx, session, "attribute manager lookup", &lookup_scope,
                  &lookup_edit));
    state->manager_value = 77u;
    ASSERT_EQ(NMO_OK,
              nmo_object_edit_set_parameter_value_ex(
...
REGISTER_TEST(workspace_edit, manager_edits_reject_cross_section_strings);
