...
TEST(lua_bindings_plan, plan_module_builds_edit_plan)
{
    ...
    assert_lua_ok(
        ...
        "plan.add_node(p, 3, '055B29FE-662D5CA0', 'Lua Plan 2D Text', { manager_entry = { policy = 'create_missing', manager = 'message' } })\n"
...
TEST(lua_bindings_plan, plan_module_executes_behavior_link_dry_run)
{
    ...
    assert_lua_ok(
        ...
        "assert(report.diff.graph_edge_diff.created[1].before == nil)\n"
        "assert(report.diff.graph_edge_diff.created[1].after.from_io_id == 5)\n"
        "assert(report.diff.graph_edge_diff.created[1].after.to_io_id == 2)\n"
        "assert(report.diff.graph_edge_diff.created[1].after.activation_delay == 3)\n"
...
TEST(lua_bindings_plan, plan_parameter_value_accepts_manager_entry_policy)
{
    ...
    assert_lua_ok(
        ...
        "local script = behavior.script_at(doc, 1)\n"
        "local p = plan.new()\n"
        "plan.add_node(p, script.script_id, 'A20E8D5B-DF002150', 'Lua Plan Send Message', { manager_entry = { policy = 'create_missing', manager = 'message', manager_guid = '{466A0FAC-00000000}', key = 'LuaPlanNodeMessage' } })\n"
        "plan.set_parameter_value_from_handle(p, 1, 'input_param:Message', 'LuaPlanCreatedMessage', { manager_entry = { policy = 'create_missing', manager = 'message', manager_guid = '{466A0FAC-00000000}', key = 'LuaPlanParamMessage' } })\n"
        "local report = plan.execute(p, ws, { dry_run = true })\n"
        "assert(report.ok == true)\n"
        "assert(report.dry_run == true)\n"
        "assert(#report.operations == 2)\n"
...
TEST(lua_bindings_plan, plan_manager_entry_reports_policy_path)
{
    nmo_lua_runtime_t *runtime = nmo_lua_runtime_create();
    ASSERT_NOT_NULL(runtime);
    ...
    assert_lua_error_contains(
        ...
        "local plan = require('nmo.plan')\n"
        "local p = plan.new()\n"
        "plan.set_parameter_value(p, 5, 'hello', { manager_entry = { policy = 'bad' } })\n",
