...
TEST(lua_bindings_plan, plan_module_builds_edit_plan)
{
    ...
    assert_lua_ok(
        ...
        "plan.add_node(p, 3, '055B29FE-662D5CA0', 'Lua Plan 2D Text', { manager_entry = { policy = 'create_missing', schema = 'message' } })\n"
...
TEST(lua_bindings_plan, plan_parameter_value_accepts_manager_entry_policy)
{
    ...
    assert_lua_ok(
        ...
        "local script = behavior.script_at(doc, 1)\n"
        "local p = plan.new()\n"
        "plan.add_node(p, script.script_id, 'A20E8D5B-DF002150', 'Lua Plan Send Message', { manager_entry = { policy = 'create_missing', schema = 'message', manager_guid = '{466A0FAC-00000000}', key = 'LuaPlanNodeMessage' } })\n"
        "plan.set_parameter_value_from_handle(p, 1, 'input_param:Message', 'LuaPlanCreatedMessage', { manager_entry = { policy = 'create_missing', schema = 'message', manager_guid = '{466A0FAC-00000000}', key = 'LuaPlanParamMessage' } })\n"
        "local report = plan.execute(p, ws, { dry_run = true })\n"
        "assert(report.ok == true)\n"
        "assert(report.dry_run == true)\n"
        "assert(#report.operations == 2)\n"
