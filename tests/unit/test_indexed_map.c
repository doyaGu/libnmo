...
TEST(indexed_map, basic) {
    ...
    ASSERT_EQ(nmo_indexed_map_get(map, &key1, &retrieved), NMO_OK);
...
TEST(indexed_map, multiple) {
    ...
    for (uint32_t i = 0; i < 50; i++) {
        uint32_t key = i;
        uint32_t value = 0;
        ASSERT_EQ(nmo_indexed_map_get(map, &key, &value), NMO_OK);
...
TEST(indexed_map, remove) {
    ...
    // Remove some entries
    uint32_t key5 = 5;
    ASSERT_EQ(nmo_indexed_map_remove(map, &key5), NMO_OK);
...
TEST(indexed_map, lifecycle_hooks) {
    ...
    ASSERT_EQ(NMO_OK, nmo_indexed_map_remove(map, &key2));
    ...
    ASSERT_EQ(NMO_OK, nmo_indexed_map_insert(map, &key1, &val1));
    ASSERT_EQ(NMO_OK, nmo_indexed_map_remove(map, &key1));
    ASSERT_EQ(key_total, 6u);
    ASSERT_EQ(value_total, 160u);
