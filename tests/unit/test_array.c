...
TEST(buffer, init_with_allocator_basic) {
    ...
    nmo_array_t buffer;
    nmo_result_t result = nmo_array_init(&buffer, sizeof(uint32_t), 4, &allocator);

    ASSERT_EQ(result.code, NMO_OK);
    ASSERT_NOT_NULL(buffer.data);
    ASSERT_EQ(buffer.capacity, 4);
    /* Allocator-backed array */
...
TEST(buffer, allocator_growth_and_dispose) {
    nmo_allocator_t allocator = nmo_allocator_default();
    nmo_array_t buffer;
    nmo_result_t result = nmo_array_init(&buffer, sizeof(uint32_t), 0, &allocator);
    ASSERT_EQ(NMO_OK, result.code);
