...
TEST(lua_bindings_object, object_module_exposes_document_scoped_queries_and_edges)
{
    ...
    assert_lua_ok(
        ...
        "assert(first ~= nil)\n"
        "local first_class = object.class_id(first)\n"
        "local first_class_count = object.count_class(doc, first_class)\n"
        "assert(first_class_count >= 1)\n"
        "assert(object.at_class(doc, first_class, 1) ~= nil)\n"
        "assert(object.at_class(doc, first_class, first_class_count + 1) == nil)\n"
