...
TEST(chunk_16bit, dword_as_words_basic) {
    ...
    for (size_t i = 0; i < test_count; i++) {
        uint32_t value;
        nmo_status_t parse_result = nmo_chunk_parser_read_dword_as_words(parser, &value);
...
TEST(chunk_16bit, dword_as_words_array_helper) {
    ...
    nmo_status_t parse_result = nmo_chunk_parser_read_dword_array_as_words(parser,
                                                                           decoded,
                                                                           value_count);
    ASSERT_EQ(parse_result, NMO_OK);
...
TEST(chunk_16bit, buffer_nosize_lendian16_basic) {
    ...
    nmo_status_t parse_result = nmo_chunk_parser_read_buffer_nosize_lendian16(parser,
                                                                              value_count,
                                                                              read_data);
    ASSERT_EQ(parse_result, NMO_OK);
...
TEST(chunk_16bit, mixed_operations) {
    ...
    uint32_t dword_val;
    nmo_status_t parse_result = nmo_chunk_parser_read_dword(parser, &dword_val);
    ASSERT_EQ(parse_result, NMO_OK);
...
TEST(chunk_16bit, error_handling) {
    ...
    uint32_t val;
    nmo_status_t parse_result = nmo_chunk_parser_read_dword_as_words(parser, &val);
    ASSERT_EQ(parse_result, NMO_OK);
...
TEST(chunk_16bit, virtools_compatibility) {
    ...
    uint32_t read_count;
    nmo_status_t parse_result = nmo_chunk_parser_read_dword_as_words(parser, &read_count);
    ASSERT_EQ(parse_result, NMO_OK);
