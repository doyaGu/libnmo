...
TEST(cli, validate_references_normalize_preserves_clean_chunks) {
    ...
    snprintf(args, sizeof(args), "diff chunks \"%s\" \"%s\"",
             fixture, output);
    result = run_cli_capture(args);
    ASSERT_NOT_NULL(result.output);
    ASSERT_EQ(NMO_CLI_EXIT_SUCCESS, result.exit_code);
...
TEST(cli, debug_probe_to_io_links_probe_output) {
    ...
    snprintf(args, sizeof(args),
             "-f json debug probe 2d-text --behavior 237 --from-io 232 "
             "--to-io 233 --name LinkedProbe --text \"loading trace\" "
             "\"%s\" --dry-run",
             NMO_TEST_DATA_FILE("Ballance/base.cmo"));
    yyjson_doc *doc = run_cli_json(args);
    ASSERT_NOT_NULL(doc);
    ...
    yyjson_val *operations = yyjson_obj_get(data, "operations");
    ASSERT_TRUE(operations && yyjson_is_arr(operations));
    ASSERT_EQ(4u, yyjson_arr_size(operations));
    yyjson_val *out_link_op = yyjson_arr_get(operations, 3);
    ASSERT_TRUE(out_link_op && yyjson_is_obj(out_link_op));
    ASSERT_STR_EQ("add_behavior_link",
                  yyjson_get_str(yyjson_obj_get(out_link_op, "op")));
    ASSERT_EQ(0u, (uint32_t)yyjson_get_uint(
                      yyjson_obj_get(out_link_op, "status")));
    ASSERT_TRUE(yyjson_get_uint(yyjson_obj_get(out_link_op, "result_id")) != 0u);
    yyjson_doc_free(doc);
}
...
TEST(cli, debug_probe_remove_link_inserts_probe) {
    ...
    snprintf(args, sizeof(args),
             ...
             "--from-io 232 --to-io 150 --name InsertedProbe "
             "--text \"loading trace\" \"%s\" --dry-run",
             NMO_TEST_DATA_FILE("Ballance/base.cmo"));
    yyjson_doc *doc = run_cli_json(args);
    ASSERT_NOT_NULL(doc);
    ASSERT_STR_EQ(json_envelope_command(doc), "debug.probe");
    ...
    yyjson_val *remove_op = yyjson_arr_get(operations, 0);
    ASSERT_TRUE(remove_op && yyjson_is_obj(remove_op));
    ASSERT_STR_EQ("remove_behavior_link",
                  yyjson_get_str(yyjson_obj_get(remove_op, "op")));
    ASSERT_EQ(0u, (uint32_t)yyjson_get_uint(
                      yyjson_obj_get(remove_op, "status")));
    yyjson_val *deleted = yyjson_obj_get(data, "deleted_objects");
    ASSERT_TRUE(deleted && yyjson_is_arr(deleted));
    ASSERT_TRUE(yyjson_arr_size(deleted) >= 1u);
    yyjson_doc_free(doc);
}
...
TEST(cli, debug_probe_message_logger_inserts_on_selected_message_link) {
    ...
    snprintf(args, sizeof(args),
             ...
             "--name MessageLoggerProbe --text \"message trace\" "
             "\"%s\" --dry-run",
             NMO_TEST_DATA_FILE("Ballance/base.cmo"));
    yyjson_doc *doc = run_cli_json(args);
    ASSERT_NOT_NULL(doc);
...
TEST(cli, debug_probe_parameter_logger_connects_source_parameter) {
    ...
    snprintf(args, sizeof(args),
             ...
             "--name ParameterLoggerProbe --parameter 234 \"%s\" --dry-run",
             NMO_TEST_DATA_FILE("Ballance/base.cmo"));
    yyjson_doc *doc = run_cli_json(args);
    ASSERT_NOT_NULL(doc);
    ASSERT_STR_EQ(json_envelope_command(doc), "debug.probe");
    ...
    ASSERT_NOT_NULL(data);
    ASSERT_TRUE(yyjson_get_bool(yyjson_obj_get(data, "ok")));
    ASSERT_STR_EQ("parameter-logger",
                  yyjson_get_str(yyjson_obj_get(data, "probe_kind")));
    yyjson_val *operations = yyjson_obj_get(data, "operations");
    ASSERT_TRUE(operations && yyjson_is_arr(operations));
    ASSERT_EQ(2u, yyjson_arr_size(operations));
    ASSERT_STR_EQ("add_node",
                  yyjson_get_str(yyjson_obj_get(
                      yyjson_arr_get(operations, 0), "op")));
    ASSERT_STR_EQ("connect_parameter",
                  yyjson_get_str(yyjson_obj_get(
                      yyjson_arr_get(operations, 1), "op")));
    ASSERT_TRUE(yyjson_get_uint(yyjson_obj_get(
                    yyjson_arr_get(operations, 1), "result_id")) != 0u);
...
TEST(cli, debug_probe_data_cell_logger_inserts_on_selected_write_link) {
    ...
    snprintf(args, sizeof(args),
             "-f json debug probe data-cell-logger --behavior 3880 "
             "--dataarray 6067 --row 0 --col 1 --write-link 3874 "
             "\"%s\" --dry-run",
             NMO_TEST_DATA_FILE("Ballance/base.cmo"));
    yyjson_doc *doc = run_cli_json(args);
    ASSERT_NOT_NULL(doc);
...
TEST(cli, debug_probe_rejects_invalid_probe_options) {
    struct probe_case {
        ...
        {
            "debug probe message-logger --behavior 2172 --message-node 1667 "
            "--remove-link 2156 "
            "\"" NMO_TEST_DATA_FILE("Ballance/base.cmo") "\" --dry-run",
