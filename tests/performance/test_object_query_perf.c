...
static void populate_repeated_trigram_repo(
    nmo_object_repository_t *repo,
    const nmo_allocator_t *allocator,
    size_t object_count)
{
    for (size_t i = 0; i < object_count; i++) {
        ...
        snprintf(name, sizeof(name),
                 "Repeat_%05zu_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
                 i);
...
TEST(object_query_perf, text_index_deduplicates_repeated_object_trigrams)
{
    const size_t object_count = 30000;
    ...
    nmo_object_repository_t *repo = nmo_object_repository_create(&allocator);
    ASSERT_NOT_NULL(repo);
    populate_repeated_trigram_repo(repo, &allocator, object_count);
    ...
    double indexed_ms =
        benchmark_indexed(&qctx, &substring_query, iterations, &indexed_matches);
    ASSERT_EQ(linear_matches, indexed_matches);
    assert_speedup("substring repeated trigram dedupe", linear_ms, indexed_ms, 3.0);
