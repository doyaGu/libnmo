...
static void patch_add_guid_json(yyjson_mut_doc *doc,
                                yyjson_mut_val *obj,
                                const char *key,
                                nmo_guid_t guid) {
    char guid_text[32];
...
static void patch_add_optional_id_json(yyjson_mut_doc *doc,
                                       yyjson_mut_val *obj,
                                       const char *key,
                                       nmo_object_id_t id) {
    if (id != 0) {
...
static const char *patch_io_kind_to_string(nmo_script_edit_io_kind_t kind) {
    return kind == NMO_SCRIPT_EDIT_IO_OUTPUT ? "output" : "input";
}

static const char *patch_parameter_kind_to_string(
...
static const char *patch_script_interface_mode_to_string(
...
static const char *patch_fold_interface_mode_to_string(
...
static void patch_add_ref_json(yyjson_mut_doc *doc,
                               yyjson_mut_val *obj,
    ...
    nmo_cli_json_add_str_safe(doc, obj, handle_key, handle_name);
}

static char *patch_bytes_to_hex(const uint8_t *bytes, size_t byte_count) {
    ...
    if (!bytes || byte_count > (SIZE_MAX - 1u) / 2u) {
        return NULL;
    }
...
static yyjson_mut_val *patch_fold_maps_to_json(
    yyjson_mut_doc *doc,
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        patch_add_optional_id_json(doc, item, "old_id", maps[i].old_id);
        patch_add_optional_id_json(doc, item, "new_id", maps[i].new_id);
...
static yyjson_mut_val *patch_edit_op_to_manifest_json(
    yyjson_mut_doc *doc,
    ...
    yyjson_mut_obj_add_str(
        doc, obj, "op", nmo_cli_edit_report_op_kind_string(op->kind));

    switch (op->kind) {
        case NMO_EDIT_OP_SET_PARAMETER_VALUE:
            if (op->data.set_value.has_parameter_ref) {
                patch_add_ref_json(
                    doc, obj, "parameter_operation", "parameter_handle",
            ...
            nmo_cli_json_add_str_safe(doc, obj, "value",
        ...
        case NMO_EDIT_OP_SET_PARAMETER_BYTES: {
            ...
            char *hex = patch_bytes_to_hex(op->data.set_bytes.bytes,
        ...
        case NMO_EDIT_OP_ADD_NODE:
            ...
            patch_add_guid_json(doc, obj, "guid",
                                op->data.add_node.bb_guid);
            nmo_cli_json_add_str_safe(doc, obj, "name",
        ...
        case NMO_EDIT_OP_ADD_BEHAVIOR_LINK:
            ...
            if (op->data.add_link.has_from_io_ref) {
                patch_add_ref_json(
                    doc, obj, "from_operation", "from_handle",
            ...
            if (op->data.add_link.has_to_io_ref) {
                patch_add_ref_json(
                    doc, obj, "to_operation", "to_handle",
        ...
        case NMO_EDIT_OP_ADD_PARAMETER:
            ...
            yyjson_mut_obj_add_str(
                doc, obj, "kind",
                patch_parameter_kind_to_string(
                    op->data.add_parameter.kind));
            patch_add_guid_json(doc, obj, "type_guid",
        ...
        case NMO_EDIT_OP_CONNECT_PARAMETER:
            ...
            if (op->data.connect_parameter.has_target_parameter_ref) {
                ...
                yyjson_mut_obj_add_uint(
                    doc, obj, "target_id",
                    (uint64_t)op->data.connect_parameter
                        .target_parameter_id);
            }
            break;
        ...
        case NMO_EDIT_OP_ADD_OPERATION:
            ...
            patch_add_guid_json(doc, obj, "operation_guid",
        ...
        case NMO_EDIT_OP_INTERFACE_POLICY:
            ...
            yyjson_mut_obj_add_str(
                doc, obj, "mode",
                patch_script_interface_mode_to_string(
        ...
        case NMO_EDIT_OP_FOLD: {
            ...
            patch_add_guid_json(doc, obj, "guid", desc->block_guid);
            nmo_cli_json_add_str_safe(doc, obj, "name", desc->name);
            ...
            yyjson_mut_obj_add_str(
                doc, obj, "interface",
                patch_fold_interface_mode_to_string(desc->interface_mode));
            yyjson_mut_obj_add_val(
                doc, obj, "inputs",
                patch_fold_maps_to_json(doc, desc->input_maps,
                                        desc->input_map_count));
            yyjson_mut_obj_add_val(
                doc, obj, "outputs",
                patch_fold_maps_to_json(doc, desc->output_maps,
                                        desc->output_map_count));
            yyjson_mut_obj_add_val(
                doc, obj, "parameters",
                patch_fold_maps_to_json(doc, desc->parameter_maps,
...
static void patch_add_normalized_manifest_json(yyjson_mut_doc *doc,
    ...
    yyjson_mut_obj_add_uint(doc, manifest, "version", 2u);
    nmo_cli_json_add_str_safe(doc, manifest, "input", plan->input);
    nmo_cli_json_add_str_safe(doc, manifest, "output", plan->output);

    size_t count = nmo_edit_plan_count(plan->edit_plan);
    for (size_t i = 0; i < count; ++i) {
        const nmo_edit_op_t *op = nmo_edit_plan_get(plan->edit_plan, i);
        yyjson_mut_val *op_obj = patch_edit_op_to_manifest_json(doc, op);
        if (op_obj) {
            yyjson_mut_arr_add_val(ops, op_obj);
        }
    }

    yyjson_mut_obj_add_val(doc, manifest, "operations", ops);
...
static int patch_parse_set_parameter_bytes(yyjson_val *op_obj,
    ...
    if (has_parameter_id == has_parameter_ref) {
        fprintf(stderr,
                "Error: set_parameter_bytes requires either parameter_id or "
                "parameter_operation plus parameter_handle\n");
        free(bytes);
        return NMO_CLI_EXIT_ARG_ERROR;
    }
...
static int patch_parse_optional_operation_ref(
    yyjson_val *obj,
    const char *id_key,
    const char *operation_key,
    const char *handle_key,
    ...
    bool *out_has_ref) {
    yyjson_val *id_val = yyjson_obj_get(obj, id_key);
    yyjson_val *operation_val = yyjson_obj_get(obj, operation_key);
    yyjson_val *handle_val = yyjson_obj_get(obj, handle_key);
    bool has_id = id_val != NULL;
    bool has_ref = operation_val != NULL || handle_val != NULL;

    if (out_operation_index) {
    ...
    if (has_id) {
        fprintf(stderr,
                "Error: %s requires either %s or %s plus %s\n",
                where, id_key, operation_key, handle_key);
        return NMO_CLI_EXIT_ARG_ERROR;
    }
    if (!operation_val || !yyjson_is_uint(operation_val) ||
        yyjson_get_uint(operation_val) == 0) {
        fprintf(stderr, "Error: Missing or invalid %s in %s\n",
                operation_key, where);
        return NMO_CLI_EXIT_ARG_ERROR;
    }
    if (!handle_val || !yyjson_is_str(handle_val) ||
...
static int patch_parse_add_operation(yyjson_val *op_obj,
    ...
    size_t in1_ref_index = 0u;
    size_t in2_ref_index = 0u;
    size_t out_ref_index = 0u;
    const char *in1_ref_handle = NULL;
    const char *in2_ref_handle = NULL;
    const char *out_ref_handle = NULL;
    bool has_in1_ref = false;
    bool has_in2_ref = false;
    bool has_out_ref = false;
    rc = patch_parse_optional_operation_ref(
        op_obj, "in1_id", "in1_operation", "in1_handle",
        "add_operation operation", &in1_ref_index, &in1_ref_handle,
        &has_in1_ref);
    if (rc != NMO_CLI_EXIT_SUCCESS) {
        return rc;
    }
    rc = patch_parse_optional_operation_ref(
        ...
        "add_operation operation", &in2_ref_index, &in2_ref_handle,
        &has_in2_ref);
    if (rc != NMO_CLI_EXIT_SUCCESS) {
        return rc;
    }
    rc = patch_parse_optional_operation_ref(
        ...
        "add_operation operation", &out_ref_index, &out_ref_handle,
        &has_out_ref);
    if (rc != NMO_CLI_EXIT_SUCCESS) {
        return rc;
    }
    ...
    nmo_status_t st =
        (has_in1_ref || has_in2_ref || has_out_ref)
