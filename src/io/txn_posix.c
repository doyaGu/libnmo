...
struct nmo_txn_handle {
    ...
    nmo_txn_state state;             /**< Current transaction state */
    nmo_allocator allocator;         /**< Allocator for memory management */
};
 ...
 */
nmo_txn_handle* nmo_txn_open(const nmo_txn_desc* desc) {
    if (!desc || !desc->path) {
        errno = EINVAL;
        return NULL;
    }
    ...
    nmo_txn_handle* txn = (nmo_txn_handle*)nmo_alloc(&allocator,
                                                      sizeof(nmo_txn_handle),
                                                      _Alignof(nmo_txn_handle));
    if (!txn) {
        return NULL;
    }
 ...
 */
nmo_result_t nmo_txn_write(nmo_txn_handle* txn, const void* data, size_t size) {
    if (!txn) {
        nmo_error* err = NMO_ERROR(NULL, NMO_ERR_INVALID_ARGUMENT,
    ...
    if (txn->state != NMO_TXN_STATE_ACTIVE) {
        nmo_error* err = NMO_ERROR(NULL, NMO_ERR_INVALID_STATE,
    ...
    while (total_written < size) {
        ...
        if (n < 0) {
            ...
            nmo_error* err = NMO_ERROR(NULL, NMO_ERR_CANT_WRITE_FILE,
 ...
 */
nmo_result_t nmo_txn_commit(nmo_txn_handle* txn) {
    if (!txn) {
        nmo_error* err = NMO_ERROR(NULL, NMO_ERR_INVALID_ARGUMENT,
                                   NMO_SEVERITY_ERROR, "Transaction handle is NULL");
        return nmo_result_error(err);
 ...
 */
nmo_result_t nmo_txn_rollback(nmo_txn_handle* txn) {
    if (!txn) {
        nmo_error* err = NMO_ERROR(NULL, NMO_ERR_INVALID_ARGUMENT,
                                   NMO_SEVERITY_ERROR, "Transaction handle is NULL");
        return nmo_result_error(err);
