 ...
 * Get field type name
 */
const char *nmo_field_type_name(nmo_field_type_t type) {
    switch (type) {
 ...
 * Get field type size
 */
size_t nmo_field_type_size(nmo_field_type_t type) {
    switch (type) {
    case NMO_FIELD_INT8:
 ...
 */
int nmo_field_descriptor_validate(const nmo_field_descriptor_t *field) {
    if (field == NULL) {
 ...
 */
int nmo_schema_descriptor_validate(const nmo_schema_descriptor_t *schema) {
    if (schema == NULL) {
        return NMO_ERR_INVALID_ARGUMENT;
    }
