                                       ...
                                       uint32_t flags)
{
    ...
    if (nmo_field_uses_pointer_array_storage(field)) {
        ...
        while ((elem = yyjson_arr_iter_next(&iter)) != NULL) {
            ...
            nmo_status_t st = import_array_element_value(
                dest, field, field_type, registry, storage_arena, scratch_arena, elem, elem_size, flags);
            if (st != NMO_OK) {
                result->errors++;
                return st;
            }
    ...
    if (field->flags & NMO_FIELD_REPEATED) {
        ...
        if (elem_size > sizeof(stack_buf)) {
            elem_buf = nmo_arena_alloc(scratch_arena, elem_size, 8);
            if (!elem_buf) {
                result->errors++;
                return NMO_ERR_NOMEM;
            }
        ...
        while ((elem = yyjson_arr_iter_next(&iter)) != NULL) {
            ...
            st = import_array_element_value(
                elem_buf, field, field_type, registry, storage_arena, scratch_arena, elem, elem_size, flags);
            if (st != NMO_OK) {
    ...
    if (field->flags & NMO_FIELD_POINTER) {
        ...
        if ((field_type->category & NMO_TYPE_CATEGORY_STRUCT) && yyjson_is_obj(json_val)) {
            void *scratch_pointee = nmo_arena_alloc(
                scratch_arena,
                field_type->size,
                field_type->alignment > 0 ? field_type->alignment : 8);
            if (!scratch_pointee) {
                result->errors++;
                return NMO_ERR_NOMEM;
            }
            memset(scratch_pointee, 0, field_type->size);
        ...
        nmo_status_t st = import_scalar_value(scratch_pointee, &tmp_field, registry, json_val, false);
        if (st == NMO_OK) {
            if (!dry_run) {
                void *pointee = nmo_arena_alloc(
                    storage_arena,
                ...
                memcpy(pointee, scratch_pointee, field_type->size);
                *(void **)fptr = pointee;
            }
            result->fields_written++;
        } else {
    ...
    nmo_import_result_t *result)
{
    ...
    if (field->size != sizeof(nmo_array_t)) {
        ...
        if (!dry_run) {
            ...
            if (expected_size > 0) {
                persistent = nmo_arena_alloc(storage_arena, expected_size, 8);
                if (!persistent) {
                    result->errors++;
                    return NMO_ERR_NOMEM;
                }
    ...
    uint32_t flags)
{
    ...
    if (strcmp(kind, "array") == 0) {
        ...
        if (array_items_have_semantic_values(items)) {
            return import_field_value(
                state, owner_type, field, registry, storage_arena, scratch_arena, items, result, flags);
        }
        if ((raw_hex && yyjson_is_str(raw_hex)) || count == 0) {
            return import_array_raw_hex(
                state, owner_type, field, field_type, storage_arena, scratch_arena, raw_hex, count, dry_run, result);
        }
        result->errors++;
        return NMO_ERR_INVALID_FORMAT;
    }
    ...
    if (strcmp(kind, "pointer") == 0) {
        ...
        void *scratch_pointee = nmo_arena_alloc(
            scratch_arena, field_type->size, field_type->alignment > 0 ? field_type->alignment : 8);
        if (!scratch_pointee) {
            result->errors++;
            return NMO_ERR_NOMEM;
        }
        nmo_status_t st = decode_raw_hex_exact(raw_hex, scratch_pointee, field_type->size);
        if (st != NMO_OK) {
            result->errors++;
            return st;
        }
        if (!dry_run) {
            void *pointee = nmo_arena_alloc(
                storage_arena, field_type->size, field_type->alignment > 0 ? field_type->alignment : 8);
            if (!pointee) {
                result->errors++;
                return NMO_ERR_NOMEM;
            }
    ...
    return import_field_value(
        state, owner_type, field, registry, storage_arena, scratch_arena, value, result, flags);
}
    ...
    while ((item = yyjson_arr_iter_next(&iter)) != NULL) {
        ...
        for (size_t lvl = 0; lvl < level_count; ++lvl) {
            ...
            nmo_status_t st = import_snapshot_field_value(
                lvl_state, lvl_type, field, registry, storage_arena, scratch_arena, item, result, flags);
...
static nmo_status_t object_edit_import_json_impl(
    ...
    nmo_import_result_t *result)
{
    ...
    if (!workspace || !registry || !scratch_arena || !json_data) {
        NMO_RETURN_ERROR(NMO_ERR_INVALID_ARGUMENT, NMO_SEVERITY_ERROR,
                         "nmo_object_edit_import_json: NULL argument");
    }
    ...
    while ((entry = yyjson_arr_iter_next(&arr_iter)) != NULL) {
        ...
        void *state = NULL;
        nmo_arena_t *storage_arena = NULL;
        nmo_class_id_t pending_create_cid = 0;

        /* Object not found -- create or skip */
        if (!obj) {
            if (flags & NMO_IMPORT_CREATE_MISSING) {
                if (class_name) {
                    pending_create_cid = nmo_type_query_class_id_from_name(registry, class_name);
                }
                if (pending_create_cid == 0) {
                    result->errors++;
                    continue;
                }

                type = nmo_type_registry_find_by_class_id_inherited(registry, pending_create_cid);
                ...
                if ((flags & NMO_IMPORT_DRY_RUN) != 0u) {
                    state = nmo_arena_alloc(
                        scratch_arena,
                        type->size,
                        type->alignment > 0 ? type->alignment : 8);
                    if (state == NULL) {
                        yyjson_doc_free(doc);
                        return NMO_ERR_NOMEM;
                    }
                    memset(state, 0, type->size);
                } else {
                    nmo_status_t edit_st =
                        nmo_workspace_edit_begin(workspace, "object.import", &edit);
                    if (edit_st != NMO_OK) {
                        result->errors++;
                        continue;
                    }
        ...
        /* Get mutable state */
        if (obj != NULL) {
            state = nmo_object_get_state(obj);
            storage_arena = nmo_object_get_storage_arena(obj);
        }
        if (!state || (((flags & NMO_IMPORT_DRY_RUN) == 0u) && storage_arena == NULL)) {
            if (edit != NULL) {
                nmo_workspace_edit_rollback(edit);
            }
            result->errors++;
        ...
        if ((flags & NMO_IMPORT_DRY_RUN) == 0u && edit == NULL) {
            nmo_status_t edit_st =
                nmo_workspace_edit_begin(workspace, "object.import", &edit);
            if (edit_st != NMO_OK) {
                result->errors++;
            ...
            if (type->size > 0u) {
                edit_st = nmo_workspace_edit_snapshot_bytes(edit, state, type->size);
                if (edit_st != NMO_OK) {
                    nmo_workspace_edit_rollback(edit);
                    result->errors++;
                    continue;
        ...
        if (edit != NULL) {
            ...
            nmo_status_t edit_st = nmo_workspace_edit_commit(edit);
            if (edit_st != NMO_OK) {
                yyjson_doc_free(doc);
                return edit_st;
            }
    ...
    nmo_import_result_t *result)
{
    ...
    nmo_document_t *document = nmo_workspace_get_document(workspace);
    nmo_context_t *ctx = document != NULL ? nmo_document_get_context(document) : NULL;
    const nmo_type_registry_t *registry =
    ...
    nmo_arena_t *scratch_arena = nmo_arena_create(NULL, 0);
    if (scratch_arena == NULL) {
        return NMO_ERR_NOMEM;
    }

    nmo_status_t status = object_edit_import_json_impl(
        workspace,
        registry,
        scratch_arena,
        json_data,
