    ...
    const char *name)
{
    ...
    if (nmo_hash_table_get(registry->by_name, &name, &type).code == NMO_OK) {
        return type;
    }
    ...
    nmo_class_id_t class_id)
{
    ...
    const nmo_schema_type_t *type = NULL;
    if (nmo_indexed_map_get(registry->by_class_id, &class_id, &type).code == NMO_OK) {
        return type;
    }
    ...
    nmo_guid_t guid)
{
    ...
    const nmo_schema_type_t *type = NULL;
    if (nmo_hash_table_get(registry->by_guid, &guid, &type).code == NMO_OK) {
        return type;
    }
