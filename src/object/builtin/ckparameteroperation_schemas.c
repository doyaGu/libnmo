...
commit:
    {
        ...
        nmo_ref_check_class(
            &decoded.in1.ref, repository, types, NMO_CID_PARAMETERIN);
        nmo_ref_check_class(
            &decoded.in2.ref, repository, types, NMO_CID_PARAMETERIN);
        nmo_ref_check_class(
            &decoded.out.ref, repository, types, NMO_CID_PARAMETEROUT);
    }
    *out_state = decoded;
    NMO_RETURN_OK();
}
    ...
    if (is_file) {
        ...
        if (has_legacy_dummy) {
            NMO_RETURN_IF_ERROR(nmo_ref_write_sequence_item(
                out_chunk, &in_state->legacy_prefix_ref));
        }
        if (ref_count >= 1u) {
            NMO_RETURN_IF_ERROR(nmo_ref_write_sequence_item(
