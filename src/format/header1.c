    ...
    nmo_arena* arena)
{
    ...
    header->object_count = read_u32_le(data + *pos);
    *pos += 4;
    ...
    size_t objects_size = header->object_count * sizeof(nmo_object_desc);
    header->objects = (nmo_object_desc*)nmo_arena_alloc(arena, objects_size, 8);
    ...
    for (uint32_t i = 0; i < header->object_count; i++) {
        nmo_object_desc* obj = &header->objects[i];

        /* Read file ID (may have bit 23 set for reference-only) */
        CHECK_BUFFER_SIZE(*pos, 4, size);
        obj->file_id = read_u32_le(data + *pos);
        *pos += 4;
        ...
        /* Read parent ID */
        CHECK_BUFFER_SIZE(*pos, 4, size);
        obj->parent_id = read_u32_le(data + *pos);
        *pos += 4;

        /* Read class ID */
        CHECK_BUFFER_SIZE(*pos, 4, size);
        obj->class_id = read_u32_le(data + *pos);
        *pos += 4;

        /* Read name length (including null terminator) */
        CHECK_BUFFER_SIZE(*pos, 4, size);
        uint32_t name_len = read_u32_le(data + *pos);
        *pos += 4;

        /* Validate name length */
        if (name_len == 0) {
            return nmo_result_error(NMO_ERROR(NULL, NMO_ERR_INVALID_ARGUMENT,
                NMO_SEVERITY_ERROR, "Object name length cannot be zero"));
        }

        /* Read name string */
        CHECK_BUFFER_SIZE(*pos, name_len, size);
        obj->name = (char*)nmo_arena_alloc(arena, name_len, 1);
        if (obj->name == NULL) {
            return nmo_result_error(NMO_ERROR(NULL, NMO_ERR_NOMEM,
        ...
        obj->name[name_len - 1] = '\0'; /* Ensure null termination */
        *pos += name_len;
    }
    ...
    nmo_arena* arena)
{
    /* Read category count */
    CHECK_BUFFER_SIZE(*pos, 4, size);
    uint32_t category_count = read_u32_le(data + *pos);
    *pos += 4;
    ...
    for (uint32_t i = 0; i < category_count; i++) {
        ...
        *pos += 4;
        ...
        CHECK_BUFFER_SIZE(*pos, 4, size);
        uint32_t guid_count = read_u32_le(data + *pos);
        *pos += 4;
    ...
    *pos = saved_pos;
    ...
    size_t deps_size = total_plugins * sizeof(nmo_plugin_dep);
    header->plugin_deps = (nmo_plugin_dep*)nmo_arena_alloc(arena, deps_size, 8);
    ...
    for (uint32_t i = 0; i < category_count; i++) {
        /* Read category type */
        CHECK_BUFFER_SIZE(*pos, 4, size);
        uint32_t category = read_u32_le(data + *pos);
        *pos += 4;
        ...
        *pos += 4;
        ...
        for (uint32_t j = 0; j < guid_count; j++) {
            ...
            nmo_plugin_dep* dep = &header->plugin_deps[plugin_index++];
            dep->category = category;
            dep->guid.d1 = read_u32_le(data + *pos);
            *pos += 4;
            dep->guid.d2 = read_u32_le(data + *pos);
            *pos += 4;
    ...
    nmo_arena* arena)
{
    ...
    /* Initialize header */
    memset(header, 0, sizeof(nmo_header1));
    ...
    if (pos + 4 <= size) {
        header->included_file_count = read_u32_le(buffer + pos);
        pos += 4;

        /* Read included file size (also always 0) */
        if (pos + 4 <= size) {
            uint32_t included_size = read_u32_le(buffer + pos);
            pos += 4;
    ...
    size_t* pos)
{
    ...
    if (*pos + 4 > buffer_size) {
        return nmo_result_error(NMO_ERROR(NULL, NMO_ERR_BUFFER_OVERRUN,
            NMO_SEVERITY_ERROR, "Buffer too small for object count"));
    }
    write_u32_le(buffer + *pos, header->object_count);
    *pos += 4;

    /* Write each object descriptor */
    for (uint32_t i = 0; i < header->object_count; i++) {
        const nmo_object_desc* obj = &header->objects[i];

        /* Write file ID (with reference flag if set) */
        if (*pos + 4 > buffer_size) {
            return nmo_result_error(NMO_ERROR(NULL, NMO_ERR_BUFFER_OVERRUN,
        ...
        write_u32_le(buffer + *pos, file_id);
        *pos += 4;

        /* Write parent ID */
        if (*pos + 4 > buffer_size) {
            return nmo_result_error(NMO_ERROR(NULL, NMO_ERR_BUFFER_OVERRUN,
                NMO_SEVERITY_ERROR, "Buffer too small for parent ID"));
        }
        write_u32_le(buffer + *pos, obj->parent_id);
        *pos += 4;
        ...
        write_u32_le(buffer + *pos, obj->class_id);
        *pos += 4;

        /* Calculate name length (including null terminator) */
        uint32_t name_len = obj->name ? (uint32_t)(strlen(obj->name) + 1) : 1;
        ...
        write_u32_le(buffer + *pos, name_len);
        *pos += 4;
    ...
    size_t* pos)
{
    ...
    write_u32_le(buffer + *pos, num_categories);
    *pos += 4;
    ...
    for (uint32_t cat = 0; cat < 5; cat++) {
        ...
        *pos += 4;
        ...
        write_u32_le(buffer + *pos, category_counts[cat]);
        *pos += 4;
        ...
        for (uint32_t i = 0; i < header->plugin_dep_count; i++) {
            if (header->plugin_deps[i].category == cat) {
                ...
                write_u32_le(buffer + *pos, header->plugin_deps[i].guid.d1);
                *pos += 4;
                write_u32_le(buffer + *pos, header->plugin_deps[i].guid.d2);
                *pos += 4;
            }
        }
    }
 ...
 */
static size_t calculate_serialize_size(const nmo_header1* header) {
    size_t size = 0;
    ...
    for (uint32_t i = 0; i < header->object_count; i++) {
        ...
        size += 4; /* class_id */
        size += 4; /* name_len */
        uint32_t name_len = header->objects[i].name ? (uint32_t)(strlen(header->objects[i].name) + 1) : 1;
        size += name_len;
    }
    ...
    nmo_arena* arena)
{
    ...
    write_u32_le(buffer + pos, 0); /* included_file_count */
    pos += 4;
    write_u32_le(buffer + pos, 0); /* included_size */
    pos += 4;
 ...
 */
void nmo_header1_free(nmo_header1* header) {
    if (header == NULL) {
        return;
    }
