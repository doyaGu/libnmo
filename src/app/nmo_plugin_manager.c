...
struct nmo_plugin_manager {
    ...
    nmo_plugin_instance_info_t *instances;
...
static int nmo_plugin_manager_reserve(nmo_plugin_manager_t *manager, size_t needed) {
    ...
    if (manager->instance_capacity >= needed) {
        return NMO_OK;
    }

    size_t new_capacity = manager->instance_capacity ? manager->instance_capacity : 4;
    while (new_capacity < needed) {
        new_capacity *= 2;
    }

    size_t bytes = new_capacity * sizeof(nmo_plugin_instance_info_t);
    nmo_plugin_instance_info_t *new_block = (nmo_plugin_instance_info_t *) nmo_alloc(manager->allocator, bytes, alignof(nmo_plugin_instance_info_t));
    if (new_block == NULL) {
        return NMO_ERR_NOMEM;
    }

    if (manager->instances != NULL && manager->instance_count > 0) {
        memcpy(new_block, manager->instances, manager->instance_count * sizeof(nmo_plugin_instance_info_t));
    }

    if (manager->instances != NULL) {
        nmo_free(manager->allocator, manager->instances);
    }

    memset(new_block + manager->instance_count, 0, (new_capacity - manager->instance_count) * sizeof(nmo_plugin_instance_info_t));

    manager->instances = new_block;
    manager->instance_capacity = new_capacity;
    return NMO_OK;
}
...
static int nmo_plugin_manager_find_index_by_guid(const nmo_plugin_manager_t *manager, nmo_guid_t guid) {
    ...
    for (size_t i = 0; i < manager->instance_count; ++i) {
        if (nmo_guid_equals(manager->instances[i].plugin->guid, guid)) {
            return (int)i;
        }
    }
...
nmo_plugin_manager_t *nmo_plugin_manager_create(nmo_context_t *ctx) {
    ...
    nmo_plugin_manager_t *manager = (nmo_plugin_manager_t *) nmo_alloc(allocator, sizeof(nmo_plugin_manager_t), alignof(nmo_plugin_manager_t));
    if (manager == NULL) {
        return NULL;
    }
...
void nmo_plugin_manager_destroy(nmo_plugin_manager_t *manager) {
    ...
    for (size_t i = 0; i < manager->instance_count; ++i) {
        nmo_plugin_instance_info_t *info = &manager->instances[i];
    ...
    nmo_free(manager->allocator, manager);
}
...
int nmo_plugin_manager_register(
    ...
    size_t required = manager->instance_count + desc->plugin_count;
    ...
    for (size_t i = 0; i < desc->plugin_count; ++i) {
        ...
        nmo_plugin_instance_info_t *slot = &manager->instances[manager->instance_count++];
...
int nmo_plugin_manager_load_library(
    ...
    size_t required = manager->instance_count + plugin_count;
    int reserve_result = nmo_plugin_manager_reserve(manager, required);
    if (reserve_result != NMO_OK) {
        nmo_shared_library_close(library);
        return reserve_result;
    ...
    size_t first_index = manager->instance_count;
    ...
    if (manager->instance_count > first_index) {
        manager->instances[first_index].flags |= NMO_PLUGIN_INSTANCE_FLAG_OWNS_LIBRARY;
    } else {
        nmo_shared_library_close(library);
    }
...
const nmo_plugin_instance_info_t *nmo_plugin_manager_get_plugins(
    ...
    if (out_count != NULL) {
        *out_count = manager->instance_count;
    }
...
const nmo_plugin_t *nmo_plugin_manager_find_by_guid(
    ...
    return manager->instances[index].plugin;
}
