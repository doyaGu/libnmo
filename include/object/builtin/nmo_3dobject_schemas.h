 ...
 */
typedef struct nmo_3dobject_state {
    nmo_3dentity_state_t entity;  ///< Parent CK3dEntity state
} nmo_3dobject_state_t;

NMO_API nmo_status_t nmo_3dobject_deserialize(
    void *instance,
    nmo_chunk_t *chunk,
    const nmo_type_descriptor_t *type,
    void *context);

NMO_API nmo_status_t nmo_3dobject_serialize(
    const void *instance,
    nmo_chunk_t *out_chunk,
    const nmo_type_descriptor_t *type,
    void *context);

NMO_DECLARE_OBJECT_SCHEMA(nmo_3dobject_vtable, nmo_register_3dobject_type)

NMO_API nmo_status_t nmo_3dobject_prepare_dependencies(
    void *instance,
    const nmo_type_descriptor_t *type,
    void *context);

NMO_API nmo_status_t nmo_3dobject_remap_dependencies(
    void *instance,
    const nmo_type_descriptor_t *type,
    void *context);
